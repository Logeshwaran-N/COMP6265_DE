...
def calculate_price(query: ParsedQuery, selected_plan: CandidatePlan, conflicts: List[Dict[str, Any]], role: str, purpose: str) -> Dict[str, Any]:
    ...
    for ds in datasets:
        for col in _columns_for_dataset(ds, query):
            if CATALOGUE[ds]["columns"].get(col, {}).get("pii"):
                sensitive_fee += 2.0
    returned = max(int(selected_plan.estimated_rows_returned or 0), 0)
    if returned > 5000:
        row_tier_fee = 4.0
    elif returned > 1000:
        row_tier_fee = 2.0
    ...
    return {
        ...
        "row_tier_note": f"Estimated returned rows={selected_plan.estimated_rows_returned}; bulk access fee increases for larger history queries.",
